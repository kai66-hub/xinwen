...
from toutiao_backend.config.cahe_conf import get_json_cache, set_cache

CATEGORIES_KEY = "news:categories"      #给缓存数据起的一个名字，带不带:都行
...
# 避免所有key同时过期 引起缓存雪崩
# 这个函数只能存 List [Dict] 格式（多条数据）不能存单条数据
