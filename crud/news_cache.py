...
async def get_categories(db: AsyncSession, skip: int = 0, limit: int = 100):
    ...
    # 写入缓存
    if categories:
        categories = jsonable_encoder(categories)         #jsonable_encoder就是把orm格式变成能看懂的格式比如列表字典啥的
                                                          #就是如果categories单条就是{}，多条就是[{},{}]自动转的
...
async def get_news_list(db: AsyncSession, category_id: int, skip: int = 0, limit: int = 10):
    ...
    if cached_list:
        ...
        return [News(**item) for item in cached_list]   #redis读取的转成orm
        # 就是把缓存字典形式的数据转成orm,News(xx=xx,xx=xx)就是**的作用
    ...
    if news_list:
        ...
        #就是遍历列表 → 把每个 ORM 对象 → 按 Pydantic 模型的规范 → 转换成标准字典！
